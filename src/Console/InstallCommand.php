...
class InstallCommand extends Command
{
    ...
    /**
     * Rename file extensions in the given path.
     *
     * @return void
     */
    protected function renameFileExtensions($old, $new, $path)
    {
        $files = (new Finder)
            ->in($path)
            ->name("*.{$old}");
        ...
        foreach ($files as $file) {
            rename($file->getPathname(), preg_replace("/\.{$old}$/", ".{$new}", $file->getPathname()));
        }
    }
}
